<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="./css/PaginaLogin.css">
        <script src="https://kit.fontawesome.com/9ab9988c3d.js" crossorigin="anonymous"></script>
        <link rel="shortcut icon" type="image/jpg" href="./images/favicon.png" />
        <title>CyMaT</title>

    </head>
    <body>
        <header>
            <ul>
                <li>
                    <a class="logo" href="./index_client.html">
    
                        <!-- <span class="header_icon_logo"> <i class="fas fa-clinic-medical"></i> </span> -->
                        <span class="header_icon_logo"> &#128757; </span>
                        <img class="logo" src="./images/favicon.png" alt="CyMaT">
                        <!-- <span class="header_icon"> <i class="fas fa-calendar-plus"></i> </span>
                            <span class="header_text">Logo</span> -->
                    </a>
                </li>
    
    
                <li>
                    <a class="header_button" href="./PaginaCalendarClientNelogat.html">
                        <!-- <span class="header_icon"> <i class="fas fa-calendar-plus"></i> </span> -->
                        <span class="header_icon"> &#128197; </span>
                        <span class="header_text">Program</span>
                    </a>
                </li>
    
                <li>
                    <a class="header_button" href="./PaginaFormularClientNelogat_intial.html">
                        <!-- <span class="header_icon"> <i class="fa fa-check-square"></i> </span>	 -->
                        <span class="header_icon"> &#128193; </span>
                        <span class="header_text">Formular</span>
                    </a>
                </li>
    
    
                <li>
                    <a class="header_button">
                        <!-- <span class="header_icon"> <i class="fas fa-comments"></i> </span>
                        <span class="header_text">Istoric</span> -->
                    </a>
                </li>
    
                <li>
                    <a class="header_button">
                        <!-- <span class="header_icon"> <i class="fas fa-user"></i> </span> -->
                        <span class="header_icon"> &#128100; </span>
                        <span class="header_text">User</span>
                    </a>
                    <ul class="header_button__login">
                        <li>
                            <a class="header_button" href="./PaginaSignupClient.html">
                                <!-- <span class="header_icon"> <i class="fas fa-key"></i> </span> -->
                                <span class="header_icon"> &#9919; </span>
    
                                <span class="header_text">Sign up</span>
                            </a>
                        </li>
                        <li>
                            <a class="header_button" href="./PaginaLoginClient.html">
                                <!-- <span class="header_icon"> <i class="fas fa-sign-out-alt"></i> </span>	 -->
                                <span class="header_icon"> &#9094; </span>
    
                                <span class="header_text">Log in</span>
                            </a>
                        </li>
                    </ul>
    
                </li>
    
            </ul>
        </header>


        <div class="login">
            <div class="login__content">
                

                <div class="login__forms">
                    <form action="../MvcPart/app/controllers/login.php" class="login__registre" id="login-in" method="post">
                        <h1 class="title">Sign In</h1>
                        <?php if (isset($_GET['error'])) { ?>
                            <p class="error"><?php echo $_GET['error']; ?></p>
                        <?php } ?>

                        <?php if (isset($_GET['success'])) { ?>
                            <p class="success"><?php echo $_GET['success']; ?></p>
                        <?php } ?>
                        <div class="input-group">
                            <input type="text" name="name" placeholder="Username"  minlength="5" maxlength="20" class="login-input" required>
                        </div>
    
                        <div class="input-group">
                            <input type="password" name="password" placeholder="Password" minlength="5" maxlength="20" class="login-input" required>
                        </div>

                        <button type="submit" class="login__button" name="login_button">Sign In</button>

                        <div>
                            <span class="login__account">Don't have an Account ?</span>
                            <span class="login__signin" id="sign-up">Sign Up</span>
                        </div>
                    </form>

<!--                    <form action="./PaginaGeneralaClient.html" class="login__create none" id="login-up" method="post">-->
<!--                        <h1 class="title">Create Account</h1>-->
<!--    -->
<!--                        <div class="input-group">-->
<!--                            -->
<!--                            <input type="text" class="login-input" minlength="5" maxlength="20" placeholder="Username" required>-->
<!--                        </div>-->
<!--    -->
<!--                        <div class="input-group">-->
<!--                            -->
<!--                            <input type="email" class="login-input" minlength="10" maxlength="30" placeholder="Email" required>-->
<!--                        </div>-->

<!--                        <div class="input-group">-->
<!--                            -->
<!--                            <input type="password" class="login-input" minlength="5" maxlength="20" placeholder="Password" required>-->
<!--                        </div>-->

<!--                        &lt;!&ndash; <a class="login__button">Sign Up</a> &ndash;&gt;-->
<!--                        <button type="submit" class="login__button" name="login_button" value="Sign Up">Sign Up</button>-->


<!--                        <div>-->
<!--                            <span class="login__account">Already have an Account ?</span>-->
<!--                            <span class="login__signup" id="sign-in">Sign In</span>-->
<!--                        </div>-->

<!--                        -->
<!--                    </form>-->
<!--                    -->
<!--                    <form action="./PaginaGeneralaClient.html" class="login__change__pass none" id="login-forgot" method="post">-->
<!--                        <h1 class="title">Change Password</h1>-->
<!--    -->
<!--                        <div class="input-group">-->
<!--                            -->
<!--                            <input type="email" class="login-input" minlength="10" maxlength="30" placeholder="Email" required>-->
<!--                        </div>-->
<!--    -->

<!--                        <div class="input-group">-->
<!--                            -->
<!--                            <input type="password" class="login-input" minlength="5" maxlength="20" placeholder="New_Password" required>-->
<!--                        </div>-->

<!--                        &lt;!&ndash; <a href="./PaginaGeneralaClient.html" class="login__button">Save</a> &ndash;&gt;-->
<!--                        <button type="submit" class="login__button" name="login_button" value="Save">Save</button>-->

<!--                        <div>-->
<!--                            <span class="login__account">You want to give up ?</span>-->
<!--                            <span class="login__pass" id="sign-in1">Sign In</span>-->
<!--                        </div>-->
<!--                    </form>-->
<!--                -->
                </div>

                
            </div>
        </div>

        
        <script src="./js/login.js"></script>
    </body>
</html>